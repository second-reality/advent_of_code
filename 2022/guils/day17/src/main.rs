...
fn update_cache(plane: &mut Plane, cache: &mut Cache, jet_idx: i64, rock_idx: i64) -> UniqueId {
	...
	for x in 0..cache.1 {
	    if plane.contains_key(&(x, y)) { chars.push('#') } else { chars.push(' ') }}}
...
fn falling_loop(rocks: &[Rock], jets: &[Jet], max_rocks: i64) -> i64 {
	...
	loop {
	    ...
	    let next_pos = (if *jet == '<' { cmp::max(0, pos.0 - 1) } else { cmp::min(pos.0 + 1, width-size.0) }, pos.1);
	...
	if cached {
		...
		if let Some(r) = unique_map.get(&id) {
		    ...
		    let (div, rem) = ((max_rocks - n_rocks) / delta_r, (max_rocks - n_rocks) % delta_r);
		    let delta_height = (height - history[*r as usize - 1]) * div + history[*r as usize + rem as usize - 1] - history[*r as usize - 1];
		    height += delta_height;
		    break;
		} else {

...
fn get_all_succs(start: VId, graph: &VGraph, rates: &VRates) -> VGraph {
    ...
    let mut to_visit = graph.keys().map(|(a, _)| *a).sorted().dedup()
	.filter(|a| *a == start || rates.get(a).is_some())
	.map(|a| (a, a, 0)).collect::<VecDeque<_>>();
...
fn prepare_graph(valves: &VMap) -> (VNamesIds, VNames, VGraph, VRates) {
    let names_ids: VNamesIds = valves.keys().sorted().enumerate().map(|(id, s)| (s.clone(), id)).collect();
...
fn dyn_visit(graph: &VGraph, rates: &VRates, cache: &mut VMem, current: VId, time: i32, pressure: i32, path: &VPath) -> VResult {
    ...
    let succs = graph.iter().filter_map(|((a, b), d)| if *a == current && rates.contains_key(b) { Some((*b, *d)) } else { None }).collect_vec();
    for (next, dist) in succs.iter() {
	if time - *dist > 0 {
	    let rate = *rates.get(next).unwrap();
	    let mut new_rates = rates.clone();
	    new_rates.remove(next);
	    let (ntime, npressure, full_path) = dyn_visit(graph, &new_rates, cache, *next, time - *dist - 1, pressure + (time - *dist - 1) * rate, &new_path);
	    if ntime == 0 {
...
fn dyn_visit_2(graph: &VGraph, rates: &VRates, cache: &mut VMem2, states: &PState2) -> PResult2 {
    let mut last_state = (states.0[states.0.len()-1],
			  states.1[states.1.len()-1]);
    if last_state.0.time == 0 && last_state.0.time == 0 {
	return last_state;
    }

    let find = |t: i32, v: &Vec<PState> | Some(v.iter().take_while(|x| x.time >= t).last()?.clone());
    ...
    let succs0 = graph.iter().filter_map(|((a, b), d)| if *a == last_state.0.node && rates.contains_key(b) { Some((*b, *d)) } else { None }).collect_vec();
    let succs1 = graph.iter().filter_map(|((a, b), d)| if *a == last_state.1.node && rates.contains_key(b) { Some((*b, *d)) } else { None }).collect_vec();
